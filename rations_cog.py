...
RATIONS_SHEET_HEADER = [
    ...
    "Time Stolen",
    "Time Alive",
    "Total Rations Gained",
    "Rations Given",
    "Rations Stolen",
]
RATIONS_SHEET_LAST_COLUMN = "L"
...
TOTAL_GAINED_HEADERS = {
    "totalrationsgained",
    "rationsgained",
    "alltimegained",
    "totalgained",
}
RATIONS_GIVEN_HEADERS = {
    "rationsgiven",
    "given",
    "totalgiven",
    "generosity",
}
RATIONS_STOLEN_HEADERS = {
    "rationsstolen",
    "stolenrations",
    "totalstolen",
    "dishonor",
...
def normalize_entry(entry: dict[str, object]) -> dict[str, object]:
    ...
    entry["total_gained"] = parse_non_negative_int(
        entry.get("total_gained"),
        entry["total"],
    )
    entry["rations_given"] = parse_non_negative_int(entry.get("rations_given"))
    entry["rations_stolen"] = parse_non_negative_int(entry.get("rations_stolen"))
    return entry
...
def write_sheet_rations(
    ...
    rows = [
        [
            ...
            int(normalized_entry["total_gained"]),
            int(normalized_entry["rations_given"]),
            int(normalized_entry["rations_stolen"]),
        ]
...
def load_sheet_rations(worksheet: gspread.Worksheet) -> dict[str, dict[str, object]]:
    ...
    if len(first_row) >= 3 and len(first_row) > 1 and str(first_row[1]).strip().isdigit():
        ...
        time_stolen_index = 7
        time_alive_index = 8
        total_gained_index = 9
        rations_given_index = 10
        rations_stolen_index = 11
        ...
        total_gained_index = find_header_index(normalized_headers, TOTAL_GAINED_HEADERS)
        rations_given_index = find_header_index(normalized_headers, RATIONS_GIVEN_HEADERS)
        rations_stolen_index = find_header_index(normalized_headers, RATIONS_STOLEN_HEADERS)
    ...
    for row in rows:
        ...
        total_gained = (
            parse_non_negative_int(row[total_gained_index], total)
            if total_gained_index is not None and len(row) > total_gained_index
            else total
        )
        rations_given = (
            parse_non_negative_int(row[rations_given_index])
            if rations_given_index is not None and len(row) > rations_given_index
            else 0
        )
        rations_stolen = (
            parse_non_negative_int(row[rations_stolen_index])
            if rations_stolen_index is not None and len(row) > rations_stolen_index
            else 0
        )
        ...
        entry = normalize_entry(
            {
                ...
                "rations_given": rations_given,
                "rations_stolen": rations_stolen,
            }
        )
        if not time_alive:
            needs_normalization = True
...
def add_entry_stat(entry: dict[str, object], stat_name: str, amount: int = 1) -> None:
    entry[stat_name] = parse_non_negative_int(entry.get(stat_name)) + max(0, amount)
...
def format_stat_leaderboard(
    ...
    ranked_entries = sorted(
        entries,
        key=lambda entry: (-int(entry[stat_name]), str(entry["name"]).lower()),
    )
    ranked_entries = [
        entry for entry in ranked_entries if int(entry[stat_name]) > 0
    ][:limit]
    ...
    lines = [
        (
            f"{index}. {entry['name']} - {int(entry[stat_name])} "
            f"{singular_label if int(entry[stat_name]) == 1 else plural_label}"
        )
...
def format_time_alive_leaderboard(
    entries: list[dict[str, object]],
    now: datetime,
    limit: int = 5,
) -> str:
    ...
    for entry in entries:
        ...
        started_at = parse_cst_datetime(entry.get("time_alive"))
        if started_at is None:
            continue

        alive_entries.append((entry, now - started_at))

    alive_entries.sort(
        key=lambda item: (-int(item[1].total_seconds()), str(item[0]["name"]).lower())
    )
    alive_entries = alive_entries[:limit]

    if not alive_entries:
        return "**Longest Time Alive**\nNo living citizens yet."

    lines = [
        f"{index}. {entry['name']} - {format_duration(duration)}"
        for index, (entry, duration) in enumerate(alive_entries, start=1)
    ]
    return "**Longest Time Alive**\n" + "\n".join(lines)
...
class RationsCog(commands.Cog):
    ...
    def release_due_laborers(self, now: datetime) -> list[str]:
        ...
        for entry in self.rations.values():
            ...
            add_entry_stat(entry, "total_gained", LABOR_PAYOUT)
    ...
    async def add_ration(
        ...
        async with self.rations_lock:
            ...
            previous_total_gained = int(entry["total_gained"])
            entry["total"] = previous_total + 1
            add_entry_stat(entry, "total_gained")
            entry["dead"] = 0
            entry["labor_release"] = ""
            if previous_dead == 1:
            ...
            try:
                ...
                entry["total_gained"] = previous_total_gained
                self.record_storage_error(error)
                raise
    ...
    async def resolve_steal_selection(
        ...
        async with self.rations_lock:
            ...
            if caught:
                ...
                add_entry_stat(thief_entry, "total_gained")
                add_entry_stat(thief_entry, "rations_stolen")
                message = (
    ...
    async def build_give_prompt(
        ...
        return (
            ...
            f"Giving 1 ration has a {reward_chance}% chance for you to "
            f"gain 1 ration instead of spending one. "
            f"Select someone to give 1 ration to.{recipient_note}",
    ...
    async def resolve_give_selection(
        ...
        async with self.rations_lock:
            ...
            add_entry_stat(giver_entry, "rations_given")
            add_entry_stat(recipient_entry, "total_gained")
            rewarded = random.randint(1, 100) <= reward_chance
            if rewarded:
                giver_entry["total"] = int(giver_entry["total"]) + 2
                add_entry_stat(giver_entry, "total_gained")
        ...
        return (
            ...
            f"{'You gained 1 ration instead of spending one.' if rewarded else 'No bonus ration this time.'} "
    ...
    @app_commands.command(
        name="supremeleaderboard",
        description="Show all-time ration economy stats.",
    )
    async def supreme_leaderboard(self, interaction: discord.Interaction) -> None:
        if not await self.storage_is_available(interaction):
            return

        await self.settle_due_labor_releases()
        now = current_cst_time()
        entries = [dict(normalize_entry(entry)) for entry in self.rations.values()]

        sections = [
            "**Supreme Leaderboard**",
            format_stat_leaderboard(
                "Total Rations Gained (All-Time)",
                entries,
                "total_gained",
                "ration gained",
                "rations gained",
            ),
            format_time_alive_leaderboard(entries, now),
            format_stat_leaderboard(
                "Most Generous",
                entries,
                "rations_given",
